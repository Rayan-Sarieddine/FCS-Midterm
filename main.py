...
def displayAll(employees):#time complexity: sorted() has a time complexity of O(n*log(n)) and the for loop had O(n) which results in O(n * log(n) + n) where n in the number of employees
